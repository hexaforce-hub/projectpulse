"""
ProjectPulse — Script: Train Cost Overrun Models (Phase 4)
Trains Model B: Cost Classifier & Regressor, Evaluates vs Baselines
"""
...
from src.ml.cost_model import create_cost_classifier_candidates, create_cost_regressor_candidates
...
def main(seed=42):
    t0 = time.time()
    print("=" * 70)
    print(f"TRAINING MODEL B: COST OVERRUN RISK (Seed: {seed})")
    print("=" * 70)

    data_dir = Path("data/processed")
    models_dir = Path("models/cost")
    ...
    y_train_bin = pd.read_csv(data_dir / "y_train_future_cost_overrun_binary.csv").squeeze("columns")
    y_val_bin = pd.read_csv(data_dir / "y_val_future_cost_overrun_binary.csv").squeeze("columns")
    y_test_bin = pd.read_csv(data_dir / "y_test_future_cost_overrun_binary.csv").squeeze("columns")

    y_train_cont = pd.read_csv(data_dir / "y_train_future_cost_overrun_pct_continuous.csv").squeeze("columns")
    y_val_cont = pd.read_csv(data_dir / "y_val_future_cost_overrun_pct_continuous.csv").squeeze("columns")
    y_test_cont = pd.read_csv(data_dir / "y_test_future_cost_overrun_pct_continuous.csv").squeeze("columns")
    ...
    print("\n>>> 2. Training Candidate Cost Classifiers...")
    candidates = create_cost_classifier_candidates(seed=seed)
    candidate_metrics = {}
    best_clf = None
    best_name = None
    best_score = -1.0
    ...
    print("\n>>> 4. Training Cost Overrun Regressor...")
    ...
    for name, pipe in reg_baselines.items():
        ...
        print(f"  - Baseline '{name}': Val MAE={m['mae']:.2f}% | R2={m['r2']:.4f}")

    reg_candidates = create_cost_regressor_candidates(seed=seed)
    reg_cand_metrics = {}
    best_reg = None
    best_reg_name = None
    best_r2 = -999.0
    ...
    for name, pipe in reg_candidates.items():
        ...
        print(f"  - Candidate '{name}': Val MAE={m['mae']:.2f}% | R2={m['r2']:.4f}")
        if m["r2"] > best_r2:
            best_r2 = m["r2"]
            best_reg = pipe
            best_reg_name = name
    ...
    print(f"  - Test Regressor MAE:  {final_reg_metrics['mae']:.2f}%")
    ...
    save_model_artifact(models_dir, "cost_classifier_v1", best_clf, meta, feature_names)
    save_model_artifact(models_dir, "cost_regressor_v1", best_reg, meta, feature_names)

    with open(reports_dir / "cost_metrics.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print(f"[Model B] Training and evaluation finished in {elapsed:.2f}s.")
    print("=" * 70)
